...
dependency "azure_network_security_group" {
  ...
  # Do not query for outputs if this component will be skipped or the dependent component is not configured.
  skip_outputs = local.skip_deployment || try(!contains(keys(local.yaml_config.network), "network_security_groups"), true)
...
inputs = {
  ...
  dependencies = {
    ...
    azure_network_security_group = try(dependency.azure_network_security_group.outputs.groups, {})
  }
}
